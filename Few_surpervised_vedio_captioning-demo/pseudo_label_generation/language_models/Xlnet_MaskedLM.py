...
class XLNetDataset(Dataset):
    def __init__(self, dataset, mode, tokenizer, max_sentence_length=20):
        ...
        if os.path.exists(data_dict_path):
            ...
            for sentence in self.sentences:
                ...
                if not self.is_train:
                    ...
                    mask_id = random.randint(1, _length - 2) # 随机对一个语句掩码一个字符，用于模型测试
    ...
    def create_mini_batch(self, samples):
        ...
        _mask = pad_sequence(input_ids, batch_first=True, padding_value=-100) #(200, 21)
        input_ids = pad_sequence(input_ids, batch_first=True, padding_value=0) #(200, 21)
        attention_mask = torch.zeros(input_ids.shape,dtype=torch.float32)
        attention_mask = attention_mask.masked_fill(_mask != -100, 1) #(200, 21)

        batch_size, max_length = input_ids.shape
        label_ids = torch.zeros(batch_size,1, dtype=torch.long) #(200, 1)
        perm_mask = torch.zeros(batch_size, max_length, max_length) #(200, 21, 21)
        target_mapping = torch.zeros(batch_size, 1, max_length) #(200, 1, 21)
        for i, _mask_id in enumerate(mask_ids):
            label_ids[i,0] = input_ids[i, _mask_id] #被掩码单词的标签
            perm_mask[i,:,_mask_id] = 1 #注意力模型忽略被掩码单词
            target_mapping[i,0,_mask_id]=1 #需要预测的单词标签
    ...
    def compute_accuracy(model, dataloader, topks=[1,5,10,20,50,100],train=1):
        ...
        with torch.no_grad():
            ...
            for data in dataloader:
                ...
                _, topk_token_ids = torch.topk(logits, topk) #选取topk个单词 (200, 1, 1) (200, 1, 1)
                for i in range(topk):
                    predict_label = topk_token_ids[:,:,i] #(200, 1)
                    for j in topks:
            ...
            average_loss = total_loss / total_length #8.2700
            ...
            accuracy = correct_dict[topk]/total_length #0.039
