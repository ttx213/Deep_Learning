...
class XLNetDataset(Dataset):
    def __init__(self, dataset, mode, tokenizer, max_sentence_length=50, is_forward = True):
        ...
        max_batch = 200
        perm_mask = torch.zeros(max_batch, p_len + t_len, p_len + t_len) #(200, 52, 52)
        for i in range(p_len + t_len): #构建上三角矩阵
            if i < p_len:
        ...
        target_mapping = torch.zeros(max_batch, t_len, p_len + t_len, dtype=torch.float) #(200, 51, 52)

        for i in range(t_len): #创建对角线
    ...
    def __getitem__(self, idx):
        if self.is_forward:
            ...
            # construct input for backward language models 当模型为backward时需要将输入语句翻转
    ...
    def create_mini_batch(self, samples):
        ...
        label_ids = [s[0][1:].clone() for s in samples] #为去除<BOS>=1后的语句序列
        lengths_tensors = torch.tensor([s[1] for s in samples]) #120

        _mask = pad_sequence(input_ids, batch_first=True, padding_value=-100) #(120, 52)
        attention_mask = torch.zeros(_mask.shape,dtype=torch.float32) #(120, 52)
        attention_mask = attention_mask.masked_fill(_mask != -100, 1) #(120, 52)


        # pad input with 0 (the padded value can be arbitrary number.)
        input_ids = pad_sequence(input_ids, batch_first=True, padding_value=0) #(120, 52)
        # pad label with -100 (can not be other number.)
        label_ids = pad_sequence(label_ids, batch_first=True, padding_value=-100) #(120, 51)
        batch_size, max_length = input_ids.shape #120, 52
        perm_mask = self.perm_mask[:batch_size,:max_length,:max_length] #(120, 52, 52)

        target_mapping = self.target_mapping[:batch_size,:max_length-self.p_len,:max_length] #(120, 51 ,52)
...
class XLNetLM(torch.nn.Module):
    ...
    def forward(self,
        ...
        logits = outputs[0] #(120, 20, 32000)
        if labels is not None:
            loss = self.loss_fct(logits.reshape(-1, logits.size(-1)), labels.reshape(-1)) #3.11
            return (loss, logits)
        else:
            return (logits)
        ...
        with torch.no_grad():
            ...
            for data in dataloader:
                ...
                outputs = model(input_ids=input_ids, perm_mask=perm_mask, target_mapping=target_mapping, labels=label_ids,
                                attention_mask=attention_mask) #按从左到右顺序预测除<bos>外的所有单词
                loss = outputs[0] #3.11
                log_ppl += loss*torch.sum(lengths_tensors) #22792.1641
                total_length += torch.sum(lengths_tensors) #7200
...
if __name__ == "__main__":
    ...
    parser.add_argument('--is_forward', type=int, default=0)
