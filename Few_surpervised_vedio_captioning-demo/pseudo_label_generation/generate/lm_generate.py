...
class LMGenerate(MCMCGenerate):
    ...
    def __init__(self, device, forward_lm,  forward_lm_tokenizer, backward_lm=None, masked_lm=None,
        ...
        self.stop_tokens_tensor = torch.zeros(self.forward_lm_tokenizer.vocab_size).to(self.device) #(32000)
        self.sub_tokens_tensor = torch.zeros(self.forward_lm_tokenizer.vocab_size).to(self.device) #(32000)
        ...
        if 'GPT2' in self.forward_lm.__class__.__name__:
            ...
            # load sub tokens
            filename = '../data/tokens/xlnet_sub_tokens.txt' #载入停用词
            index = 0
            with open(filename, 'r') as fr:
                for line in fr:
                    words  = line.strip().split()
    ...
    def insert(self, input_text, input_ids, keyword_vector, keywords, subword_position, sentence_length, **kwargs ):
        ...
        # copy the input_ids avoiding the original list being changed
        old_input_text = input_text # "squirrel eating peanut shell"
        if input_ids is None:
            # input_ids does not contains bos_token_id and eos_token_id
            input_ids = self.forward_lm_tokenizer.encode(input_text, add_special_tokens=False)
        old_input_ids = input_ids #[20715, 4099, 16149, 6197]

        new_input_ids = input_ids[:] #[20715, 4099, 16149, 6197]
        new_input_ids.insert(subword_position,0) #[20715, 4099, 16149, 6197, 0]
        _keyword_vector = keyword_vector[:] #[1, 2, 3, 4]
        _keyword_vector.insert(subword_position, 0) #[1, 2, 3, 4, 0] / [0, 1, 2, 3, 4]
        ...
        prev_output_tokens = old_input_ids[:] #[20715, 4099, 16149, 6197]
        top_k_conditional_probs, log_ppls = self.language_model.sentences_probabilities(inputs_ids=candidates, subword_position = subword_position,
        prev_output_tokens = prev_output_tokens, repetition_penalty=self.repetition_penalty, ignore_last=1) #(51) (51)


        # draw a sample from probs and return the sampled index
        normalized_probs = self.normalize(top_k_conditional_probs[:self.candidate_num]) #(50)
        # _normalized_probs = self.normalize(top_k_conditional_probs[:5])
        # sampled_id = self.sample(_normalized_probs)
        # sampled_id = self.sample(normalized_probs)
        sampled_id = self.sample(normalized_probs, top_k = self.top_k ) #随机挑选一个候选句子

        new_input_ids = candidates[sampled_id] #[24, 20715, 4099, 16149, 6197]
        new_input_text = self.forward_lm_tokenizer.decode(new_input_ids, clean_up_tokenization_spaces=False) # a squirrel eating peanut shell
        ...
        p_old_state = old_input_prob #2.408^-13
        p_new_state = new_input_prob #1.163^-13

        p_new_state_given_old_state = normalized_probs[sampled_id] #0.861
        p_old_state_given_new_state = 1

        # if self.classifier_model is not None:
        #     _input_ids = []
        #     _input_ids.append([self.classifier_model_tokenizer.bos_token_id]+old_input_ids+[self.classifier_model_tokenizer.eos_token_id])#[1, 20715, 4099, 16149, 6197, 2]
        #     _input_ids.append([self.classifier_model_tokenizer.bos_token_id]+new_input_ids+[self.classifier_model_tokenizer.eos_token_id])#[1, 24, 20715, 4099, 16149, 6197, 2]

        #     _input_ids = [torch.tensor(e) for e in _input_ids] #2
        #     _mask = pad_sequence(_input_ids, batch_first=True, padding_value=-100) #(2, 7)
        #     attention_mask = torch.zeros(_mask.shape, dtype=torch.float32) #(2, 7)
        #     attention_mask = attention_mask.masked_fill(_mask != -100, 1) #(2, 7)
        #     _input_ids = pad_sequence(_input_ids, batch_first=True, padding_value=0) #(2, 7)
        #     with torch.no_grad():
        #         _input_ids = _input_ids.to(self.device) 
        #         attention_mask = attention_mask.to(self.device)
        #         outputs = self.classifier_model(input_ids=_input_ids, attention_mask=attention_mask)
        #         logits = outputs[0] #(2, 7, 4)
        #         probabilities = torch.softmax(logits, -1) #(2, 7, 4)
        #         probabilities = probabilities[:,1:,1:] #(2, 6, 3)
        ...
        _is_accept = self.is_accept(acceptance) #是否接受
        # print(_is_accept, old_input_text, self.check_sentence(new_input_text, keywords),new_input_text)
        if _is_accept and self.check_sentence(new_input_text, keywords): #查看是否所有的句子都包含关键词
            probability = new_input_prob
            perplexity = new_input_log_ppl

            sentence_length +=1
            keyword_vector.insert(subword_position, 0) #[0, 1, 2, 3, 4]
