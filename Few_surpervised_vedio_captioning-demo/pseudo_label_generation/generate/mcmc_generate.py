...
class MCMCGenerate(object):
    ...
    def check_sentence(self, sentence, keywords):
        ...
        """
        words = word_tokenize(sentence) #['a', 'squirrel', 'eating', 'peanut', 'shell']
...
class LanguageModel(object):
    ...
    def __init__(self, device, forward_lm, forward_lm_tokenizer, backward_lm=None, masked_lm=None):
        ...
        if 'XLNet' in self.forward_lm.__class__.__name__:
            ...
            # construct perm_mask
            p_len = 1  # the length of prompt words, when training LM we use <s> as the prompt word. 提示词长度
            self.p_len = p_len
            t_len = max_sentence_length + 1  # the maximum length of the target. 201
            max_batch = 200
            perm_mask = torch.zeros(max_batch, p_len + t_len, p_len + t_len) #(200, 202, 202)
            ...
            target_mapping = torch.zeros(max_batch, t_len, p_len + t_len, dtype=torch.float) #(200, 201, 202)
        ...
        self.loss_func = torch.nn.CrossEntropyLoss(ignore_index=-100, reduction='none') #当reduce为False时，返回每个batch的元素损失
    ...
    def sentences_probabilities(self, inputs_ids=None, inputs_texts = None, subword_position=-1, prev_output_tokens=None,
        ...
        with torch.no_grad():
            ...
            for input_ids in inputs_ids:
                input_ids = [self.forward_lm_tokenizer.bos_token_id] + input_ids[:] + [self.forward_lm_tokenizer.eos_token_id] #[1, 20715, 4099, 16149, 6197, 21, 2]
                # print(self.forward_lm_tokenizer.convert_ids_to_tokens(input_ids))
                lengths.append(len(input_ids) - 1) #[6]
                ids_tensors.append(torch.tensor(input_ids))

            # pad label with -100 (can not be other number.)
            labels_tensors = pad_sequence(ids_tensors, batch_first=True, padding_value=-100) #(51, 7)
            labels_tensors = labels_tensors.to(self.device)
            labels_tensors = labels_tensors[:,1:] #(51, 6)

            # pad input with 0 (the padded value can be arbitrary number.)
            ids_tensors = pad_sequence(ids_tensors, batch_first=True, padding_value=0) #(51, 7)
            ids_tensors = ids_tensors.to(self.device)

            lengths_tensors = torch.tensor(lengths).to(self.device) #(51)
            if 'XLNet' in self.forward_lm.__class__.__name__:
                batch_size, max_length = ids_tensors.shape #51, 7
                perm_mask = self.perm_mask[:batch_size, :max_length, :max_length] #(51, 7, 7)
                target_mapping = self.target_mapping[:batch_size, :max_length - self.p_len, :max_length] #(51, 6, 7)
                outputs = self.forward_lm(input_ids=ids_tensors, perm_mask=perm_mask, target_mapping=target_mapping) #target_mapping 预测除第一个单词以外的所有单词
                logits = outputs[0] #(51, 6, 32000)
            else:
                outputs = self.forward_lm(ids_tensors)
                logits = outputs[0]
                logits = logits[:, :-1, :]

            # enforce repetition penalty 强制执行重复惩罚
            if repetition_penalty != 1 and subword_position != -1:

                if ignore_last != 0: #忽略最后一个句子
                    ignore_last *= -1
                    _logits = logits[:ignore_last, subword_position, :] #(50, 32000) 选取修改的单词的概率
                else:
                    _logits = logits[:, subword_position, :]
                # construct mask tensors
                mask_tensor = torch.zeros(1, _logits.shape[-1]).to(self.device) #(1, 32000)
                for previous_token in prev_output_tokens:
                    if mask_tensor[0, previous_token] == 0: #如果该单词在之前已经出现过，则将该单词的掩码值设为重复惩罚值
                        mask_tensor[:, previous_token] = repetition_penalty
                    else:
                        mask_tensor[:, previous_token] *= repetition_penalty
                mask_tensor = mask_tensor.repeat(_logits.shape[0], 1) #(50, 32000)
                _mask_tensor = mask_tensor * _logits #(50, 32000)
                _mask_tensor_positive = _mask_tensor > 0 #(50, 32000)
                _mask_tensor_negative = _mask_tensor < 0 #(50, 32000)
                a = _logits[_mask_tensor_positive]
                b = _logits[_mask_tensor_negative]
                _logits[_mask_tensor_positive] /= mask_tensor[_mask_tensor_positive] #将重复单词的概率降低
                _logits[_mask_tensor_negative] *= mask_tensor[_mask_tensor_negative] #将非重复单词的概率提高

            # labels_tensors[:,-1]=-100
            # compute sentence probs and log_ppls
            loss_ = self.loss_func(logits.reshape(-1, logits.shape[-1]), labels_tensors.reshape(-1)) #(51, 6)
            loss_ = loss_.reshape(labels_tensors.shape) #(306)
            # print(loss_)
            ...
            #                     print(i,j,k)
            loss_ = torch.sum(loss_, dim=-1).double() #(51)
            log_ppls = (loss_/lengths_tensors).cpu().numpy() #(51)
            probs = torch.exp(-loss_).cpu().numpy() #(51)
    ...
    def construct_forward_and_backward_input_ids(self, input_ids, subword_position, include_subword_position=0):
        ...
        if include_subword_position==1:
            forward_input_ids = [self.forward_lm_tokenizer.bos_token_id] + input_ids[:subword_position+1] #[1, 20715, 4099, 16149, 6197, 0]
            backward_input_ids = [self.forward_lm_tokenizer.bos_token_id] + list(reversed(input_ids[subword_position:])) #[1, 0]
        else:
    ...
    def conditional_distribution(self, language_model, input_ids, subword_position=-1, keyword_vector=None, device=None,
        ...
        """
        _input_ids = torch.tensor([input_ids]) #[1, 20715, 4099, 16149, 6197, 0]
        input_ids = _input_ids.to(device)

        with torch.no_grad():
            if 'XLNet' in language_model.__class__.__name__:
                batch_size, max_length = input_ids.shape #1, 6
                perm_mask = self.perm_mask[:batch_size, :max_length, :max_length] #(1, 6, 6)
                target_mapping = torch.zeros(batch_size, 1, max_length) #(1, 1, 6)
                target_mapping[:, 0, -1] = 1 #预测最后一个单词，因为已经做了裁剪
                ...
                outputs = language_model(input_ids=input_ids, attention_mask=None, perm_mask=perm_mask,
                                         target_mapping=target_mapping) #此处的perm_mask设置用于模型前向注意力实现

            else:
                outputs = language_model(input_ids)
            #  shape: [1, 1, vocab_size]
            logits = outputs[0] #(1, 1, 32000)
            logits = logits[0, -1, :] #(32000)
            ...
            if stop_tokens_tensor is not None: #排除停用词
                logits = logits.masked_fill(stop_tokens_tensor > 0 , -1e8) #(32000)
            conditional_probs = torch.softmax(logits, -1) #(32000)
        return conditional_probs #(32000)
    ...
    def conditional_distribution_bidirectional_lm(self, forward_input_ids, backward_input_ids, subword_position, keyword_vector,
        ...
        backward_conditional_probs = self.conditional_distribution(self.backward_lm, backward_input_ids,subword_position, keyword_vector,
                                                                  ...
                                                                  sub_tokens_tensor = sub_tokens_tensor,
                                                                  stop_tokens_tensor=stop_tokens_tensor)
        conditional_probs = forward_conditional_probs * backward_conditional_probs #(32000) 将前向概率和后向概率相乘
        # conditional_probs = forward_conditional_probs
        if top_k != -1:
            # select the top_k probabilities and tokens
            top_k_conditional_probs, top_k_token_ids = torch.topk(conditional_probs, top_k) #50, 50
            top_k_conditional_probs = top_k_conditional_probs.cpu().numpy() #(50)
            top_k_token_ids = top_k_token_ids.cpu().numpy() #(50)
        else:
            top_k_conditional_probs = None
            top_k_token_ids = None
        conditional_probs = conditional_probs.cpu().numpy()
    ...
    def generate_candidates(self, input_ids, subword_position, keyword_vector, top_k=None, stop_tokens_tensor=None,
        ...
        if generate_candidate_method<3:
            ...
            forward_input_ids, backward_input_ids = self.construct_forward_and_backward_input_ids\
                (input_ids, subword_position, include_subword_position=include_subword_position)#[1, 20715, 4099, 16149, 6197, 0], [1, 0]
