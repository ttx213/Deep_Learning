    ...
    def compute_accuracy(model, dataloader, datasize, device,num_labels):
        ...
        with torch.no_grad():
            ...
            for data in dataloader:
                ...
                input_ids, label_ids,attention_mask = data #(100, 51) (100, 51) (100, 51)
                ...
                loss, logits = outputs[:2] #0.1103, (100, 51, 4)

                total_loss += loss*input_ids.shape[0]
                length += input_ids.shape[0]
                # compute accuracy
                predict_label = torch.argmax(logits, dim=-1) #(100, 51)
                for i in range(num_labels):
            ...
            average_loss = total_loss / datasize #0.066
            for i in range(num_labels):
...
if __name__ == "__main__":
    ...
    model_path = f'../checkpoints/{args.dataset}/xlnet_classifier/'
    ...
    try:
        ...
        model = XLNetForTokenClassification.from_pretrained('/home/wangtao/video_caption/MCMCXLNet-master/checkpoints/MSVD/xlnet', num_labels=args.num_labels)
