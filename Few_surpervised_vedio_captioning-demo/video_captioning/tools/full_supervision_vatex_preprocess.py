...
def build_vocab(imgs, params):
    ...
    # cw = dict(sorted(counts.items(), key=lambda x: x[1], reverse=True))
    cw = sorted([(count,w) for w,count in counts.items()], reverse=True)
    # json.dump(cw, open(os.path.join("/home/wangtao/video_caption/xmodaler-master/open_source_dataset_count_thr-0/vatex_dataset/word_num.json"), "w") )
...
def encode_captions(imgs, params, wtoi):
    ...
    for img in tqdm(imgs):
        ...
        for j, s in enumerate(img['final_captions']):
            input_Li = np.zeros((max_length + 1), dtype='int32')
            output_Li = np.zeros((max_length + 1), dtype='int32') - 1

            for k, w in enumerate(s):
                if k < max_length:
                    input_Li[k+1] = wtoi[w] # one shift for <BOS>
                    output_Li[k] = wtoi[w]
            input_Li[0] = 1 # one shift for <BOS>
            # input_Li[0] = 0
            seq_len = len(s)
            if seq_len <= max_length:
                output_Li[seq_len] = 0
            else:
                output_Li[max_length] = wtoi[s[max_length]]
            tokens_ids.append(input_Li)
            target_ids.append(output_Li)
        tokens_ids_array = np.array(tokens_ids)
        target_ids_array = np.array(target_ids)
        new_data = {
                "video_id": str(img_id),
                "tokens_ids": tokens_ids_array,
...
def save_pkl_file(datalist, output_dir):
    for split in datalist:
        pkl.dump(datalist[split], open(os.path.join(output_dir, "vatex_caption_anno_{}.pkl".format(split)), "wb"))
...
def main(params):
    ...
    pkl.dump(itow, open("/home/wangtao/video_caption/xmodaler-master/open_source_dataset/open_source_dataset_test/vatex/idx2word.pkl", "wb"))
    pkl.dump(wtoi, open("/home/wangtao/video_caption/xmodaler-master/open_source_dataset/open_source_dataset_test/vatex/word2idx.pkl", "wb"))
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', default='/home/wangtao/video_caption/xmodaler-master/open_source_dataset/open_source_dataset_test/vatex/dataset.json', help='input json file to process into hdf5')

    parser.add_argument('--output_dir', default='/home/wangtao/video_caption/xmodaler-master/open_source_dataset/open_source_dataset_test/vatex', help='output directory')
