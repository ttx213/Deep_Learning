...
def padding(arr, pad_token, dtype=torch.long, mask_head_tail=False):
    lens = torch.LongTensor([len(a) for a in arr]) #[17, 48]
    max_len = lens.max().item() #48
    padded = torch.ones(len(arr), max_len, dtype=dtype) * pad_token #(2, 48)*1
    mask = torch.zeros(len(arr), max_len, dtype=torch.long) #(2, 48)*0
    for i, a in enumerate(arr):
        ...
        if mask_head_tail:
            ...
            mask[i, :lens[i]] = 1
    return padded, lens, mask #(2, 48) [17, 48] (2, 48)
...
def collate_no_tokenize(arr, numericalize, fixed_span_len=None,
    ...
    pad_token = numericalize([pad])[0] #1
    plh_token = numericalize(['<plh>'])[0] #50266
    ...
    padded, lens, mask = padding(arr, pad_token, dtype=torch.long) #(2, 48) [17, 48] (2, 48)
    
    padded = padded.to(device=device)
    mask = mask.to(device=device)
    lens = lens.to(device=device)
    return padded, lens, mask, ins_pos #(2, 48) [17, 48] (2, 48) (2)


def collate_inp_mask_after_span(arr, numericalize, ins_pos_l,
                           pad="<pad>", device='cuda'):
    arr = [["<s>"] + a + ["</s>"] for a in arr]
    arr = [numericalize(a) for a in arr] #[(17), (48)]
    
    pad_token = numericalize([pad])[0] #1
    mask_token = numericalize(['<lm-mask>'])[0] #50265

    sent_indices, pad_ins_pos = [], []
    for i, pos in enumerate(ins_pos_l):
        arr[i] = arr[i][:pos[-1] + 1] + [mask_token] + arr[i][pos[-1] + 1:]
        pad_ins_pos.extend(list(range(pos[0], pos[-1] + 2))) #所有修改单词位置
        sent_indices.extend([i] * (pos[-1] + 2 - pos[0])) #每个修改单词属于哪句话
    ins_pos = [torch.tensor(sent_indices).to(device), torch.tensor(pad_ins_pos).to(device)] #[tensor([0, 0, 0, 1, 1]), tensor([ 7,  8,  9, 13, 14])]
    
    padded, lens, mask = padding(arr, pad_token, dtype=torch.long) #(2, 49) [17, 49] (2, 49)
...
def get_ngram_topk(inps_grad, mask, C, max_span_len, fixed_span_len=3):
    inps_grad = torch.where(mask.eq(1), inps_grad, torch.full_like(inps_grad, -1e16)) #删除已命名实体
    bsz, seqlen = inps_grad.size() #2, 48
    
    ngrams_b = [[] for _ in range(bsz)]
    for ngram in range(1, fixed_span_len + 1):
        ngram_grad = 0.
        for i in range(ngram):
            ngram_grad += inps_grad[:, i: i + (seqlen - ngram + 1)] #(2, 47) 计算n-grame
        
        value, position = torch.topk(ngram_grad / (ngram + C), 1, dim=1) #(2, 1) (2, 1)
    ...
    for bat, seq in enumerate(ngrams_b):
        ...
        sorted_ngrams_b.append(sorted(seq, key=lambda x: x[0], reverse=True)[0][1]) #选择得分最高短语作为将要修改的文本片段

    return sorted_ngrams_b #[[7, 8], [13]]
...
def get_entity_mask(sents, sents_id, tokenizer, add_prefix_space):
    def spanfinder(sent_id, span, mask):
    ...
    ent_mask = torch.ones_like(sents_id) #(2, 48)
    docs = list(nlp.pipe(sents, disable=["tagger", "parser"]))

    for i, doc in enumerate(docs):
        for ent in doc.ents: #分词
            if ent.label_ in ENT_TYPES:
                ...
                ent_mask[i] = spanfinder(sents_id[i], ent_id_span, ent_mask[i])
    return ent_mask
