...
class CiderScorer(object):
    ...
    def compute_cider(self):
        def counts2vec(cnts):
            ...
            return vec, norm, length #4, [14.120977412228221, 21.1083379440293, 26.55677068356685, 29.472407335680067] ,17
        ...
        def sim(vec_hyp, vec_ref, norm_hyp, norm_ref, length_hyp, length_ref):
            ...
            return val #array([0.01189014, 0.00919706, 0.        , 0.        ])

        scores = []
        for test, refs in zip(self.ctest, self.crefs):
            # compute vector for test captions
            vec, norm, length = counts2vec(test) #<defaultdict, len() = 58>
            # compute vector for ref captions
            score = np.array([0.0 for _ in range(self.n)]) #array([0., 0., 0., 0.])
            for ref in refs:
...
class Cider:
    ...
    def compute_score(self, gts, res):
        ...
        (score, scores) = self.cider_scorer.compute_score() #平均得分，全部得分
