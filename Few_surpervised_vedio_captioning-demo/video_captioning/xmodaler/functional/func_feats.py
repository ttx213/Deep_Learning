...
def pad_sequence(sequences, batch_first=False, padding_value=0.0):
    ...
    max_size = sequences[0].size() #(20, 1536)
    trailing_dims = max_size[1:] #(1536)
    max_len = max([s.size(0) for s in sequences]) #(20)
    if batch_first:
        out_dims = (len(sequences), max_len) + trailing_dims #(30, 20, 1536)
    else:
        out_dims = (max_len, len(sequences)) + trailing_dims
    
    # out_tensor = sequences[0].new_full(out_dims, padding_value) #(30, 20, 1536)

    out_tensor = torch.zeros(out_dims, dtype=sequences[0].dtype)

    for i, tensor in enumerate(sequences):
        length = tensor.size(0) #(20)
...
def pad_tensor(tensor, padding_value, use_mask):
    ...
    out = pad_sequence(tensor, batch_first=True, padding_value=padding_value) #(30, 20, 1536)

    if use_mask:
        lengths = [t.size(0) for t in tensor] #[40]
