...
class LabelSmoothing(nn.Module):
    ...
    def Forward(self, logits, targets):
        logP = F.log_softmax(logits.view(-1, logits.shape[-1]), dim=-1) #(2688, 5493)
        targets = targets.view(-1) #2688
        mask = targets >= 0
        ...
        true_dist.fill_(self.label_smoothing / (size - 1)) #利用()中的数字填充
    ...
    def forward(self, outputs_dict):
        ...
        if kfg.G_LOGITS in outputs_dict:
            logits = outputs_dict[kfg.G_LOGITS] #(128, 21, 5493) 生成语句
            targets = outputs_dict[kfg.G_TARGET_IDS] #(128, 21) 真实语句标签
