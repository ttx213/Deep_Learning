...
class RLMeanTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        bs_rewards = self.scorer(bs_outputs_dict) #{'Cider':, 'rewards':(128,)}
        ...
        sample_rewards = sample_rewards.view(-1, self.cfg.DATALOADER.SEQ_PER_SAMPLE) #(128, 1)
        sum_rewards = sample_rewards.sum(0, keepdim=True)
        mean_rewards = (sum_rewards - sample_rewards) / (sample_rewards.size(0) - 1)
        # mean_rewards = (sample_rewards.sum(0, keepdim=True) - sample_rewards) / (sample_rewards.size(1) - 1) #（128, 1)
        rewards = sample_rewards - mean_rewards #(128, 1)
        rewards = rewards.view(-1) #(128)
        ########################################################
