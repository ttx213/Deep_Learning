...
class DefaultTrainer(TrainerBase):
        ...
        with torch.no_grad():
            for data in tqdm.tqdm(test_data_loader):
                ...
                if cfg.INFERENCE.GENERATION_MODE == True:
                    ...
                    # print("GFLOPs: ", flops.total())
                    res = model(data, use_beam_search=True, output_sents=True)
                else:
                    res = model(data)

                outputs = res[kfg.OUTPUT] #['a cat is playing', 'a monkey is playing']
                for id, output in zip(ids, outputs):
                    results.append({cfg.INFERENCE.ID_KEY: int(id), cfg.INFERENCE.VALUE: output}) #[{'image_id': 1297, 'caption': 'a cat is playing'}, {'image_id': 1298, 'caption': 'a monkey is playing'}]
    ...
    def run_step(self):
        ...
        outputs_dict = self.model(data, use_beam_search=None, output_sents=False)
