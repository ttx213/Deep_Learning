...
__all__ = ["MSVDClipDataset"]

@DATASETS_REGISTRY.register() #调用自定义函数
class MSVDClipDataset:
    @configurable
    def __init__(
        self,
        stage: str,
    ...
    def load_data(self, cfg): #载入标注文件
        ...
        if self.stage == 'train':
            ...
            for data in datalist:
                if sentence_nums > 0:
                    for token_id, target_id, object_id, soft_target_id in zip(data['tokens_ids'][:sentence_nums], data['target_ids'][:sentence_nums], data['object_ids'][:sentence_nums], data['soft_target_ids'][:sentence_nums]): #(21,) (21,)
