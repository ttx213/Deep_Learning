...
class VTVTTDataset:
    ...
    def from_config(cls, cfg, stage: str = "train"):
        ann_files = {
            "train": os.path.join(cfg.DATALOADER.ANNO_FOLDER, "vt-vtt_caption_anno_train.pkl"),
            "val": os.path.join(cfg.DATALOADER.ANNO_FOLDER, "vt-vtt_caption_anno_val.pkl"),
            "test": os.path.join(cfg.DATALOADER.ANNO_FOLDER, "vt-vtt_caption_anno_test.pkl")
        }
        ret = {
            "stage": stage,
    ...
    def load_data(self, cfg): #载入标注文件
        ...
        video_list = glob.glob(os.path.join("/data16t/wangtao/dataset/TV-VTT/TV-VTT-Swin-base/", '*.npy'))
        video_ids = []
        for video_path in video_list:
            filepath, tempfilename = os.path.split(video_path)
            video_id, extension = os.path.splitext(tempfilename)
