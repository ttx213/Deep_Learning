...
__all__ = ["MSRVTTClipDataset"]

@DATASETS_REGISTRY.register() #调用自定义函数
class MSRVTTClipDataset:
    @configurable
    def __init__(
        self,
        stage: str,
    ...
    def from_config(cls, cfg, stage: str = "train"):
        ann_files = {
            "train": os.path.join(cfg.DATALOADER.ANNO_FOLDER, "msrvtt_caption_anno_clipfilter_train.pkl"),
            "val": os.path.join(cfg.DATALOADER.ANNO_FOLDER, "msrvtt_caption_anno_clipfilter_val.pkl"),
            "test": os.path.join(cfg.DATALOADER.ANNO_FOLDER, "msrvtt_caption_anno_clipfilter_test.pkl")
        }
        ret = {
            "stage": stage,
    ...
    def load_data(self, cfg): #载入标注文件
        ...
        if self.stage == 'train':
            ...
            for data in datalist:
                for token_id, target_id, object_id, soft_target_id in zip(data['tokens_ids'], data['target_ids'], data['object_ids'], data['soft_target_ids']): #(21,) (21,)
                    expand_datalist.append({
                        ...
                        'soft_target_ids': soft_target_id,
                    }) #将每个语句进行拆分
    ...
    def __call__(self, dataset_dict): #使得类实例对象可以像调用普通函数那样，以“对象名()”的形式使用
        ...
        object_ids = dataset_dict['object_ids']
        soft_target_ids = dataset_dict['soft_target_ids']
        ret.update({
            ...
            kfg.G_SOFT_TARGET_IDS: soft_target_ids,
        })
