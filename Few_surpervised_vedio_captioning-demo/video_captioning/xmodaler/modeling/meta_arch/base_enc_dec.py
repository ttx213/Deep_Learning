...
class BaseEncoderDecoder(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        *,
        ...
        self.pre_parameters = pre_parameters
    ...
    def from_config(cls, cfg):
        return {
            ...
            "vocab_size": cfg.MODEL.VOCAB_SIZE, #5492
            "max_seq_len": cfg.MODEL.MAX_SEQ_LEN, #21
            "model_type": cfg.MODEL.TYPE,
            "pre_parameters": cfg.MODEL.PRE_PARAMETERS
        }
    ...
    def forward(self, batched_inputs, use_beam_search=True, output_sents=True):
    ...
    def preprocess_batch(self, batched_inputs):
        ...
        vfeats, vmasks = pad_tensor(vfeats, padding_value=0, use_mask=True) #(30,20,1536)
        ret = { kfg.ATT_FEATS: vfeats, kfg.ATT_MASKS: vmasks}

        if kfg.MOTION_ATT_FEATS in batched_inputs[0]:
            mfeats = [x[kfg.MOTION_ATT_FEATS] for x in batched_inputs]
            if sample_per_sample > 1:
                mfeats = flat_list_of_lists(mfeats)
            mfeats, mmasks = pad_tensor(mfeats, padding_value=0, use_mask=True) #(30, 20, 2048)
            ret.update( { kfg.MOTION_ATT_FEATS: mfeats } )

        if kfg.OBJECT_ATT_FEATS in batched_inputs[0]:
            ofeats = [x[kfg.OBJECT_ATT_FEATS] for x in batched_inputs]
            if sample_per_sample > 1:
                ofeats = flat_list_of_lists(ofeats)
            ofeats, omasks = pad_tensor(ofeats, padding_value=0, use_mask=True) #(30, 20, 2048)
            ret.update( { kfg.OBJECT_ATT_FEATS: ofeats, kfg.OBJECT_ATT_MASKS: omasks } )
            # ret.update( { kfg.OBJECT_ATT_MASKS: omasks } )

        if kfg.G_OBJECT_IDS in batched_inputs[0]:
            g_object_ids = []
            for x in batched_inputs:
                x[kfg.G_OBJECT_IDS]['nouns_vec'] = torch.as_tensor(x[kfg.G_OBJECT_IDS]['nouns_vec'])
                g_object_ids.append(x[kfg.G_OBJECT_IDS])
            ret.update({ kfg.G_OBJECT_IDS: g_object_ids })

        if kfg.G_SOFT_TARGET_IDS in batched_inputs[0]:
            vocab_ids = []
            vocab_probs = []
            fillmasks = []
            for x in batched_inputs:
                vocab_ids.append(torch.as_tensor(x[kfg.G_SOFT_TARGET_IDS]['vocab_ids']).unsqueeze(0))
                vocab_probs.append(torch.as_tensor(x[kfg.G_SOFT_TARGET_IDS]['vocab_probs']).unsqueeze(0))
                fillmasks.append(torch.as_tensor(x[kfg.G_SOFT_TARGET_IDS]['fillmasks']).unsqueeze(0))
            vocab_ids = torch.cat(vocab_ids, dim=0).long() #(30, 21, 50)
            vocab_probs = torch.cat(vocab_probs, dim=0).float() #(30, 21, 50)
            fillmasks = torch.cat(fillmasks, dim=0).float() #(30, 21)
            ret.update({ kfg.VOCAB_IDS: vocab_ids, kfg.VOCAB_PROBS: vocab_probs, kfg.FILLMASKS: fillmasks})
        ...
        if kfg.SEMANTICS_IDS in batched_inputs[0]:
            ...
            semantics_ids, semantic_mask = pad_tensor(semantics_ids, padding_value=0, use_mask=True) #(2, 1) (2, 11)
            ret.update( { kfg.SEMANTICS_IDS: semantics_ids, kfg.SEMANTICS_MASK: semantic_mask} )

        if kfg.SEMANTICS_LABELS in batched_inputs[0]:
            semantics_labels = [x[kfg.SEMANTICS_LABELS] for x in batched_inputs] #(2, 11)
            semantics_labels = pad_tensor(semantics_labels, padding_value=-1, use_mask=False) #(2, 11)
            ret.update( { kfg.SEMANTICS_LABELS: semantics_labels } )

        if kfg.SEMANTICS_MISS_LABELS in batched_inputs[0]:
            semantics_miss_labels = [x[kfg.SEMANTICS_MISS_LABELS] for x in batched_inputs] #(2, 1001)
            semantics_miss_labels = pad_tensor(semantics_miss_labels, padding_value=-1, use_mask=False) #(2, 1001)
