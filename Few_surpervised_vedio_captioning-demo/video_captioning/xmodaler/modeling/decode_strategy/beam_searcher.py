...
class BeamSearcher(DecodeStrategy):
    ...
    def _bert_expand_state(self, states, selected_beam, batch_size, beam_size, cur_beam_size):
        for i in range(len(states)):
            shape = list(states[i].shape) #[128, 1, 512] / [640, 1, 512] / [640, 3, 512]
            beam = selected_beam #(128, 5)
            for _ in shape[1:]:
                beam = beam.unsqueeze(-1) #(128, 5, 1, 1) / (640, 5, 1, 1)
            states[i] = torch.gather(states[i].view(*([batch_size, cur_beam_size] + shape[1:])), 1,
                beam.expand(*([batch_size, beam_size] + shape[1:]))) #(128, 5, 1, 512) / (128, 5, 2, 512) / (128, 5, 3, 512)
            states[i] = states[i].view(*([-1, ] + shape[1:])) #(640, 1, 512) / (640, 2, 512) / (640, 3, 512)

    def _gpt2_expand_state(self, states, selected_beam, batch_size, beam_size, cur_beam_size):
        for i in range(len(states)):
            state = states[i].transpose(0, 1) #(640, 2, 12, 2, 64)
            shape = list(state.shape) #[640, 2, 12, 2, 64]
            beam = selected_beam #(128, 5)
            for _ in shape[1:]:
                beam = beam.unsqueeze(-1) #(128, 5, 1, 1, 1, 1)
            state = torch.gather(state.view(*([batch_size, cur_beam_size] + shape[1:])), 1,
                beam.expand(*([batch_size, beam_size] + shape[1:])))
            state = state.view(*([-1, ] + shape[1:])).transpose(0, 1)
            states[i] = state


    def _forward(self, batched_inputs, model):
        batch_size = batched_inputs[kfg.ATT_FEATS].size(0) #5
        out_size = batched_inputs.get('OUT_SIZE', 1) # 1
        beam_size = self.beam_size #5
        log_probs = []
        selected_words = None
        seq_logprob = torch.zeros((batch_size, 1, 1)).cuda() #(128, 1, 1)
        seq_mask = torch.ones((batch_size, beam_size, 1)).cuda() #(128, 1, 1)
        wt = Variable(torch.zeros(batch_size, dtype=torch.long).cuda()) + self.bos_token_id #(128)
        ...
        for t in range(self.max_seq_len):
            ...
            decoder_out = model.decoder(inputs) #(128, 1, 512) / (640, 2, 512) / (640, 3, 512) / (640 ,21, 512)
            # decoder_out = model.step(inputs)
            inputs.update(decoder_out)

            logit = model.predictor(inputs)[kfg.G_LOGITS] #(128, 1, 5493) / (640, 1, 5493)
            ...
            selected_beam = torch.div(selected_idx, candidate_logprob.shape[-1], rounding_mode='floor') #(128, 5)
            selected_words = selected_idx - selected_beam * candidate_logprob.shape[-1] #(128, 5)
            ...
            for key in expand_keys:
                if key in inputs:
                    states = inputs[key] #(128, 1, 512)*6 / (2, 128, 12, 1, 64)
                    if self.model_type == 'bert':
                        self._bert_expand_state(states, selected_beam, batch_size, beam_size, cur_beam_size)
                    elif self.model_type == 'gpt2':
                        self._gpt2_expand_state(states, selected_beam, batch_size, beam_size, cur_beam_size)
        ...
        return {
            kfg.IDS: batched_inputs[kfg.IDS], #[128]
            kfg.G_SENTS_IDS: outputs, #(128, 21)
            kfg.G_LOGP: log_probs #(128, 21)
        }