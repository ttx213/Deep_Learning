...
class GreedyDecoder(DecodeStrategy):
    ...
    def _forward(self, batched_inputs, model):
        batch_size = batched_inputs[kfg.ATT_FEATS].size(0) #4
        ...
        ve_out = model.visual_embed(batched_inputs) #'ATT_FEATS':(4,40,768)
        inputs.update(ve_out)

        encoder_out_v = model.encoder(inputs, mode='v') #'ATT_FEATS':(4,40,768) (4,3,40,768)
        inputs.update(encoder_out_v)
        inputs = model.decoder.preprocess(inputs)

        sents = Variable(torch.zeros((batch_size, self.max_seq_len), dtype=torch.long).cuda()) + self.eos_token_id #(4,21)
        logprobs = Variable(torch.zeros(batch_size, self.max_seq_len).cuda()) #(4,21)
        wt = Variable(torch.zeros(batch_size, dtype=torch.long).cuda()) + self.bos_token_id #(4)
        unfinished = wt.eq(wt)
        ...
        for t in range(self.max_seq_len):
            ...
            if self.model_type == 'bert':
                te_out = model.token_embed(inputs) #当bert时需要，当GPT2时不需要
            ...
            logit = model.predictor(inputs)[kfg.G_LOGITS].view(batch_size, -1) #(4, 5493)
            ...
            if is_sample:
                ...
                logP_t, wt = torch.max(logprobs_t, 1) #(4)
