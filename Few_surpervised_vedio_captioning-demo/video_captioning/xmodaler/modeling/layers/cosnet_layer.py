...
class COSJointAttention(nn.Module):
    ...
    def forward(
        ...
        v_outputs = self.attn(mixed_query_layer, v_mixed_key_layer, v_mixed_value_layer, v_attention_mask) #(128, 21, 768)
        o_outputs = self.attn(mixed_query_layer, o_mixed_key_layer, o_mixed_value_layer, o_attention_mask) #(128, 21, 768)
        vo_outputs = self.vo_proj(torch.cat([v_outputs, o_outputs], dim=-1)) #(128, 21, 768)

        # gate = self.gate_attn(torch.cat([l_outputs, vo_outputs], dim=-1))
        # gate = self.gate(gate)
        # outputs = gate * l_outputs + (1 - gate) * vo_outputs #(128, 21, 768)
        outputs = l_outputs + vo_outputs
