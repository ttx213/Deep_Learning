...
class COSNetEncoder(nn.Module):
    ...
    def forward(self, batched_inputs, mode=None):
        ...
        if mode == None or mode == 'v':
            vfeats = batched_inputs[kfg.ATT_FEATS] #(160, 50, 512)
            ext_vmasks = batched_inputs[kfg.EXT_ATT_MASKS] #(160, 1, 1 ,49)
            ext_vmasks = torch.cat([ext_vmasks[:,:,:,0:1], ext_vmasks], dim=-1) #(160, 1, 1, 50)
            ...
            for layer_module in self.layers:
                encoder_vfeats, _ = layer_module(encoder_vfeats, ext_vmasks) #(160, 50, 512)
