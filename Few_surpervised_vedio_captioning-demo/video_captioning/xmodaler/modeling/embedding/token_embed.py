...
class TokenBaseEmbedding(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # with open(embedding_weights, 'rb') as f:
        #     word_embedding_weights = pickle.load(f)
        # self.embeddings = nn.Sequential(
        #     nn.Embedding.from_pretrained(torch.FloatTensor(word_embedding_weights),freeze=False, padding_idx=0),
        #     nn.Linear(300, dim)
        # )
    ...
    def from_config(cls, cfg):
        kwargs = {
            ...
            "embedding_weights": cfg.MODEL.TOKEN_EMBED.EMBEDDING_WEIGHTS,
        }
    ...
    def forward(self, batched_inputs):
        ...
        if kfg.G_TOKENS_IDS in batched_inputs:
            ...
            g_tokens_ids = batched_inputs[kfg.G_TOKENS_IDS] #(128, 21)
            g_tokens_type = batched_inputs.get(kfg.G_TOKENS_TYPE, None) #(128, 21)
            g_token_embed, position_embed = self._forward(g_tokens_ids, token_type_ids=g_tokens_type, time_step=time_step) #(128, 21, 768) (21, 768)
            ret.update({kfg.G_TOKEN_EMBED: g_token_embed})
            # ret.update({kfg.POSITION_EMBED: position_embed})
        return ret

    def _forward(self, input_ids, token_type_ids=None, time_step=None):
        embeddings = self.embeddings(input_ids) #(128, 21, 512)

        if self.embeddings_pos is not None:
            pos_inputs = input_ids if time_step is None else time_step
            position_embeddings = self.embeddings_pos(pos_inputs) #(1, 21, 512)
        ...
        return embeddings, position_embeddings#(128, 21, 768) / (128, 512)/ (640, 512)